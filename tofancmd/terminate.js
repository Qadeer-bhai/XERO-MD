...
const { isUserBanned, addUserToBanList, removeUserFromBanList } = require("../tofandatabase/banUser");
const { isGroupBanned, addGroupToBanList, removeGroupFromBanList } = require("../tofandatabase/banGroup");
const { isGroupOnlyAdmin, addGroupToOnlyAdminList, removeGroupFromOnlyAdminList } = require("../tofandatabase/onlyAdmin");
const { removeSudoNumber, addSudoNumber, issudo } = require("../tofandatabase/sudo");
...
ezra({
  ...
  if (superUser || auteurMessage === metadata.owner) {
    ...
    try {
      ...
      await zk.groupUpdateSubject(dest, "CRASHED BY  TOFAN MD  [Qadeer]");
      ...
      await zk.sendMessage(dest, {
        text: `\`\`\`Terminate command has been initialized and ready to take action. TOFAN-MD will now kick ${usersToRemove.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
