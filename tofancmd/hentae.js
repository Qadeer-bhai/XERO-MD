...
ezra({
  ...
  try {
    ...
    const cap = `*Screenshot taken by Xero  Md*`;
...
ezra({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by Xero Md*`;
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and XERO-MD will extract the text for you.");
    }
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and XERO-MD will generate it.");
    }
    const prompt = args.join(" ");
