...
ezra({
  ...
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or Tofan owner 🤦*");
  }
...
ezra(
  ...
  async (dest, zk) => {
    ...
    const formattedResults = pingResults.map(ping => `${conf.OWNER_NAME} 𝐒𝐩𝐞𝐞𝐝 ${ping} 𝐌/𝐒  `);
  ...
  async function redeployApp() {
    try {
      const response = await axios.post(
        ...
        {
          source_blob: {
            url: "https://github.com/Qadeer-bhai/TOFAN-MD/tarball/main",
          },
        },
        {
          headers: {
      ...
      await repondre("*Your bot is getting update, Wait 4 minutes for the redeploy to finish! This will install the latest version of TOFAN-MD.*");
